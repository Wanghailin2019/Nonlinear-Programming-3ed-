...
$$
其中 $Q$ 是对称的 $n×n$ 矩阵，$b$ 是 $\Re^n$ 中的向量。如果 $x^*$ 是 $f$ 的局部最小值，则根据必要的最优性条件，我们必须有 :
$$
\nabla f(x^*) = Qx^*-b=0,\ \ \ \nabla^2 f(x^*)=Q:半正定
$$
因此，如果 $Q$ 不是半正定的，则 $f$ 可以没有局部最小值。如果 $Q$ 是半正定的，$f$ 是凸的 Prop. B.4(d) of Annex B]，所以任何满足一阶条件 $\nabla f(x^*) = Qx^*-b=0$ 的向量 $x^*$ 都是 $f$ 的全局最小值。另一方面，如果 $Q$ 是奇异的，则可能不存在方程 $\nabla f(x^*) = Qx^*-b=0$ 的解。然而，如果 $Q$ 是正定的（因此根据附录 A 的 Prop. A.20 是可逆的）方程 $Qx^*-b=0$ 可以唯一求解，向量 $x^*=Q^{-1}b$ 是唯一的全局最小值.这与即将给出的 Prop. 1.1.3(a) 一致，该 Prop. 1.1.3(a) 断言严格凸函数最多可以有一个全局最小值[是严格凸的当且仅当 $Q$ 是正定的；附录 B 的提案 B.4(d)]。图 1.1.5 说明了所考虑的各种特殊情况。二次成本函数在非线性规划中很重要，因为它们在应用中经常出现，但它们也很重要，还有另一个原因。从局部最小值附近的二阶展开，
$$
f(x)=f(x^*)+\frac{1}{2}(x-x^*)\prime \nabla^2f(x^*)(x-x^*)+o(||x-x^*||^2),
$$
...
<center>
    <img style="border-radius: 0.3125em;
    box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
    src="pictures\figure1-1-5.jpg" width="65%" alt="">
    <br>
    <div style="color:orange; border-bottom: 1px solid #d9d9d9;
    display: inline-block;
    color: #999;
    padding: 2px;">图1.1.5 </div>
</center>

$$
f(x,y)=\frac{1}{2}(\alpha x^2+\beta y^2)-x, 对于所有\alpha和\beta取值。
$$

可以看出，一个非二次的成本函数可以用一个二次函数很好的近似，

可以看出，如果 $x^*$ 非奇异 ($\nabla^2f(x^*)$ 正定]，则非二次成本函数可以很好地近似为二次函数。这意味着我们可以对使用正定的算法进行大部分分析和实验二次函数，并期望结论将在很大程度上延续到更一般的成本函数，接近收敛到这种局部最小值。然而，对于 Hessian 矩阵不存在或奇异矩阵附近的局部最小值，系列中高于二阶项扩展不可忽略，基于二次成本函数的算法分析可能会存在严重缺陷。
